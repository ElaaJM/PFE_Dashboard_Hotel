...
const Header: React.FC<HeaderProps> = ({ onLogout, logo, userInfo }) => {
  ...
  return (
    <header className="sticky top-0 z-10 bg-white border-b border-bizerta-lightgold/20 py-3 px-4 sm:px-6">
      ...
      <Link to="/dashboard" className="text-lg font-semibold text-bizerta-charcoal hidden md:block">
        Resort Performance
      </Link>
        </div>
        ...
        <Button
            ...
            onClick={() => navigate('/csv-table')}
          >
            <FileSpreadsheet  className="h-5 w-5 mr-1" />
            <span className="hidden sm:inline">CSV's Table</span>
          </Button>
